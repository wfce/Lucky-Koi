...
export const WalletButton: React.FC<{ name: string, icon: React.ReactNode, installed: boolean, onClick: () => void, disabled?: boolean }> = ({ name, icon, installed, onClick, disabled }) => (
  <button 
    onClick={onClick}
    disabled={disabled}
    className={`w-full flex items-center justify-between p-4 bg-zinc-950/50 border border-white/5 rounded-2xl transition-all group shadow-inner relative overflow-hidden ${disabled ? 'opacity-50 cursor-not-allowed' : 'hover:bg-zinc-900 active:scale-[0.98] hover:border-amber-500/20'}`}
  >
...
export const CardIconBox: React.FC<{ icon: React.ReactNode, title: string, desc: string, color: 'emerald' | 'orange' | 'amber' | 'red' }> = ({ icon, title, desc, color }) => {
  ...
  return (
    <div className="glass-card p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] border border-white/5 space-y-4 sm:space-y-6 shadow-xl hover:translate-y-[-4px] transition-all duration-300 group">
      <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-2xl flex items-center justify-center ${themes[color]} border shadow-lg group-hover:scale-110 transition-transform`}>
        {icon}
      </div>
      <div className="space-y-2">
