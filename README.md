...
*   **⚖️ 动态权重算法:** 奖金比例根据用户持仓量在 50% - 100% 之间动态计算，确保公平与激励并存。
...
*   **数据查询:** 智能路由策略，优化 7200 区块（约 6 小时）深度历史记录检索，确保 RPC 稳定性。
...
### 4. 黑暗森林 (Audit)
*   协议开放审计权限。若某用户激活后卖出代币导致持仓不足，任何人可发起“举报”。
*   被举报者有 1 小时宽限期。若超时未补足，将被系统自动移除。

## 📂 部署指南

由于本项目采用 0 后端架构，部署极其简单：
